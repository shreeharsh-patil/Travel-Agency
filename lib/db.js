...
function loadLocalDb() {
  ...
  try {
    ...
    return { users: [], places: [], reviews: [], favorites: [], reservations: [], contact_messages: [] };
  }
}
...
export async function connectToDatabase() {
  ...
  if (mongo) {
    try {
      ...
      console.warn('[db] Could not connect to MongoDB server, using local JSON database fallback.');
    }
  }
  return { client: null, db: new LocalDbWrapper() };
}
